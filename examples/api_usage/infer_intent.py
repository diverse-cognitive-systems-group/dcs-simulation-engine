#!/usr/bin/env python3
"""Manual smoke script for the Infer Intent game via APIClient."""
...
def main() -> None:
    """Start Infer Intent, print the opening turn, then take one action."""
    parser = argparse.ArgumentParser(description="Run a short Infer Intent session.")
    ...
    with APIClient(url=base_url, timeout=60.0) as api:
        ...
        with api.start_game(
            CreateGameRequest(
                ...
                source="api-example-infer-intent",
            )
        ) as run:
