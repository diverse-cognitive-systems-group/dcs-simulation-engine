...
async def test_example_run_config_parses() -> None:
    """The example config should parse against the experiment schema."""
    config = ExperimentConfig.load("example_run_config.yaml")

    assert config.assignment_protocol.strategy == "play_all_once"
    assert [form.name for form in config.forms] == ["intake", "post_play"]
