"""Live OpenRouter tests for goal-bounds scorer cases."""
...
if not os.getenv("OPENROUTER_API_KEY", "").strip():
    pytest.skip(
        "live OpenRouter credentials not configured; skipping goal-bounds scorer live tests",
...
CASES_PATH = REPO_ROOT / "tests" / "data" / "scorers" / "scorer_goal_bounds_cases.json"
...
def _cases() -> list[dict[str, Any]]:
    ...
    assert metadata.get("model_key") == "SCORER_GOAL_BOUNDS"
...
async def test_scorer_goal_bounds_live(case: dict[str, Any]) -> None:
    """Run the real goal-bounds scorer against each fixture case."""
