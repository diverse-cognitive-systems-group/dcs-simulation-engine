"""Live OpenRouter tests for shared-goal scorer cases."""
...
from dcs_simulation_engine.games.prompts import SCORER_SHARED_GOAL, build_scorer_prompt

if not os.getenv("OPENROUTER_API_KEY", "").strip():
    pytest.skip(
        "live OpenRouter credentials not configured; skipping shared-goal scorer live tests",
...
CASES_PATH = REPO_ROOT / "tests" / "data" / "scorers" / "scorer_shared_goal_cases.json"
...
def _cases() -> list[dict[str, Any]]:
    ...
    assert metadata.get("model_key") == "SCORER_SHARED_GOAL"
...
async def test_scorer_shared_goal_live(case: dict[str, Any]) -> None:
    """Run the real shared-goal scorer against each fixture case."""
    ...
    prompt = build_scorer_prompt(
        ...
        shared_goal=str(case["shared_goal"]),
        guess="Score the teamwork shown in the transcript.",
    )
