"""Live OpenRouter tests for next-action scorer cases."""
...
from dcs_simulation_engine.games.prompts import SCORER_NEXT_ACTION, build_scorer_prompt

if not os.getenv("OPENROUTER_API_KEY", "").strip():
    pytest.skip(
        "live OpenRouter credentials not configured; skipping next-action scorer live tests",
...
CASES_PATH = REPO_ROOT / "tests" / "data" / "scorers" / "scorer_next_action_cases.json"
...
def _cases() -> list[dict[str, Any]]:
    ...
    assert metadata.get("model_key") == "SCORER_NEXT_ACTION"
...
async def test_scorer_next_action_live(case: dict[str, Any]) -> None:
    """Run the real next-action scorer against each fixture case."""
    ...
    prompt = build_scorer_prompt(
        ...
        guess="Predictions are embedded in the transcript.",
    )
