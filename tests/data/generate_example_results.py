...
SESSIONS_DEF = [
    ...
    (3, "Explore",       "2026-03-25T11:40:00Z", 10, "human-gestural-improv",   "biological cell",        5, "stopping_condition_met:_runtime_seconds_>=600"),
    ...
    # Foresight ×5
    (2, "Foresight",     "2026-03-17T10:00:00Z", 12, "human-gestural-improv",   "human-gestural-improv",  3, "stopping_condition_met:_runtime_seconds_>=600"),
    (4, "Foresight",     "2026-03-20T15:00:00Z", 11, "human-normative",         "biological cell",        2, "stopping_condition_met:_turns_>=10"),
    (0, "Foresight",     "2026-03-25T11:00:00Z", 10, "human-synesthetic-tactile","eye-makeup-machine",    3, "stopping_condition_met:_runtime_seconds_>=600"),
...
# ── NPC opening scenes ────────────────────────────────────────────────────────
NPC_OPENINGS = {
    ("human-normative", "biological cell"):
    ...
    ("human-low-vision", "human-anxiety"):
    ...
    ("human-synesthetic-tactile", "eye-makeup-machine"):
    ...
    ("human-gestural-improv", "human-gestural-improv"):
    ...
    ("human-gestural-improv", "biological cell"):
    ...
    ("human-low-vision", "eye-makeup-machine"):
    ...
    ("human-normative", "eye-makeup-machine"):
    ...
    ("human-synesthetic-tactile", "biological cell"):
    ...
    ("human-gestural-improv", "eye-makeup-machine"):
...
def get_opening(pc_hid, npc_hid):
    return NPC_OPENINGS.get((pc_hid, npc_hid),
...
TURN_CONTENT = {
    ...
    ("eye-makeup-machine", 3): (
        ...
        {"liked": False, "comment": "the clack should have been louder", "doesnt_make_sense": False, "out_of_character": True, "other": False}
    ),
...
def build_events(sess_id, game, pc_hid, npc_hid, turns, term_reason, start_dt, duration_min):
    ...
    def evt(direction, event_type, event_source, content, content_format, turn_index,
            command_name=None, command_args=None, feedback=None, ts_override=None):
        nonlocal seq
        seq += 1
        ts = ts_override or t
    ...
    if game in ("Explore", "Goal Horizon"):
        ...
        if game == "Goal Horizon":
            advance(10)
            evt("inbound", "command", "user", "/predict-capabilities", "plain_text", turns + 2,
                command_name="predict-capabilities", command_args="")
            advance(1)
            evt("outbound", "command", "system",
                "What do you think this character's limits or capabilities are?\n\nDescribe the bounds you inferred in a few sentences.",
                "markdown", turns + 2, command_name="predict-capabilities", command_args="")
            advance(8)
            prediction_answers = {
            ...
            answer = prediction_answers.get(npc_hid, "The character appears to have a limited range of responses, mostly reactive rather than proactive. It cannot communicate verbally and seems to operate within narrow behavioral bounds.")
            ...
            if term_reason == "user_exit_command":
                advance(5)
                evt("inbound", "command", "user", "/exit", "plain_text", turns + 2,
                    command_name="exit", command_args="")
                advance(0)
                evt("outbound", "command", "system", "Session exited: received exit command", "markdown", turns + 2,
                    command_name="exit", command_args="")
                advance(0)
                evt("internal", "session_end", "system", "session_end: user_exit_command", "plain_text", turns + 1)
            else:
                advance(5)
                evt("internal", "session_end", "system",
                    f"session_end: {term_reason}", "plain_text", turns + 1)
        ...
        advance(10)
        evt("inbound", "command", "user", "/predict-intent", "plain_text", turns + 2,
            command_name="predict-intent", command_args="")
        advance(1)
        evt("outbound", "command", "system",
            "What do you think the character's goal or intention was during this interaction? Please describe in a few sentences.",
            "markdown", turns + 2, command_name="predict-intent", command_args="")
        advance(8)
        intent_answers = {
        ...
        for turn_i in range(1, turns + 1):
            ...
            evt("inbound", "command", "user",
                f"/predict-next {pred_text}", "plain_text", turn_i + 2,
                command_name="predict-next", command_args=pred_text)
            advance(1)
            evt("outbound", "command", "system",
...
def main():
    ...
    for p in PLAYERS:
        player_records.append({
            ...
            "consent_to_followup": {"key": "consent_to_followup", "type": "checkboxes", "label": "Follow-ups", "required": True, "pii": True,
                "answer": ["I consent to being contacted for a voluntary follow-up regarding this study."]},
            "consent_signature": {"key": "consent_signature", "type": "checkboxes", "label": "Consent Signature", "required": True, "pii": True,
                "answer": ["I confirm that the information I have provided is true and accurate. I acknowledge that I have read and understand the research consent information above, and I agree to participate. I understand that checking this box constitutes my electronic signature."]},
        })
    ...
    for s_idx, (p_idx, game, start_iso, duration_min, pc_hid, npc_hid, turns, term_reason) in enumerate(SESSIONS_DEF):
        ...
        def fmt(d): return d.strftime("%Y-%m-%dT%H:%M:%S.") + f"{d.microsecond // 1000:03d}Z"
