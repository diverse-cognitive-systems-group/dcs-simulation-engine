...
async def test_goal_horizon_scorer_provider_failure_is_terminal(patch_llm_client, _isolate_db_state, async_mongo_provider):
    """Goal Horizon should surface scorer provider failures instead of emitting a fake score."""
    session = await _make_session(async_mongo_provider)
    await session.step_async("")
