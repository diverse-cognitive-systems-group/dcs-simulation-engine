...
from dcs_utils.auto.sections import system_errors
...
def render(data: AnalysisData) -> str:
    ...
    parts.append(_full(
        _lt_violin_wait_by_player_and_game(wait_turn) + chart_caption("system_performance", "lt_wait_by_player_and_game")
    ))
...
def _build_wait_df(transcripts_df: pd.DataFrame, runs_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        msgs = transcripts_df[
            transcripts_df["event_type"] == "message"
        ][["session_id", "seq", "event_ts", "direction", "turn_index"]].dropna(subset=["event_ts"]).copy()
