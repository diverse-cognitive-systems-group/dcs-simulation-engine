...
def _report_results_cmd(
    ...
    only: Optional[List[str]] = typer.Option(
        None,
        "--only",
        help=(f"Render ONLY these section(s). Repeatable. Valid slugs: {', '.join(sorted(VALID_SECTION_SLUGS))}."),
    ),
    include: Optional[List[str]] = typer.Option(
        None,
        "--include",
        help=(f"Add section(s) to the default set. Repeatable. Valid slugs: {', '.join(sorted(VALID_SECTION_SLUGS))}."),
    ),
    exclude: Optional[List[str]] = typer.Option(
        None,
        "--exclude",
        help=(f"Remove section(s) from the default set. Repeatable. Valid slugs: {', '.join(sorted(VALID_SECTION_SLUGS))}."),
    ),
    ...
    if title is None:
        ...
        title = (run_config.get("name") if isinstance(run_config, dict) else None) or data.run.get("name") or "Results Report"
    ...
    if npc_hids:
        items = "".join(
            f'<li><a href="per_character_quality/{_html.escape(hid)}_quality_report.html">{_html.escape(hid)}</a></li>' for hid in npc_hids
        )
        links_html = (
            f'<p>Per-character quality reports are available for each NPC evaluated in this run.</p><ul class="list-unstyled">{items}</ul>'
        )
        char_links_module = types.SimpleNamespace(render=lambda _data, _h=links_html: _h)

        sim_quality_idx = next((i for i, s in enumerate(sections) if s[0] == "sim-quality"), None)
    ...
    if npc_hids:
        from dcs_simulation_engine.reporting.auto.sections.simulation_quality import build_character_quality_report
...
def _admin_publish_characters_cmd(
    ...
    r"""Publish character evaluation results from a simulation quality report.
    ...
    if hids is not None:
        ...
        if unknown:
            _console.print(
                f"ERROR: HIDs not found in report: {', '.join(unknown)}. Available: {', '.join(all_hids)}",
                style="error",
            )
            raise typer.Exit(1)
    ...
    if chars_missing_from_dev:
        _console.print(
            f"ERROR: HIDs not found in prod or dev characters.json: {', '.join(chars_missing_from_dev)}",
            style="error",
        )
        raise typer.Exit(1)
    ...
    for row in selected_rows:
        _console.print(
            f"  {row['npc_hid']}  ICF=[bold]{row['icf']:.1%}[/bold]  NCo={row['dms']:.1%}  turns={row['turns']}",
            style="dim",
        )
    ...
    for entry in new_eval_entries:
        _console.print(
            f"      + append evaluation for {entry['character_hid']}  (ICF={entry['scores']['icf']:.1%})",
            style="dim",
        )
    ...
    _console.print(
        f"[green]✔[/green] Appended {len(new_eval_entries)} evaluation(s) to {evals_path.relative_to(repo_root)}",
        style="dim",
    )
    ...
    if chars_to_add:
        ...
        _console.print(
            f"[green]✔[/green] {prod_chars_path.relative_to(repo_root)} unchanged (all characters already present)",
            style="dim",
        )
    ...
    _console.print(
        f"[green]✔[/green] Manifest updated: {len(approved)} approved character(s) → {manifest_path.relative_to(repo_root)}",
        style="dim",
    )
...
def _hitl_create_cmd(
    ...
    if out_path.exists():
        _console.print(
            f"ERROR: Test cases file already exists:\n  {out_path}\nRename or delete it before recreating.",
            style="error",
        )
        raise typer.Exit(1)
...
def _hitl_update_cmd(
    hid: str = typer.Argument(..., help="Character HID whose test cases file to update."),
    ...
    r"""Update a character's scenarios with shared history, simulator responses, and/or feedback.
    ...
    from dcs_simulation_engine.hitl.feedback import collect_feedback
    from dcs_simulation_engine.hitl.generate import scenarios_path_for
    from dcs_simulation_engine.hitl.responses import compute_status_summary, generate_responses, render_status_summary
...
def _hitl_export_cmd(
    hid: str = typer.Argument(..., help="Character HID whose test cases file to export."),
    evaluator_id: str = typer.Option(
        "evaluator",
        "--evaluator-id",
    ...
    r"""Export a completed test cases file to a standard results directory.
    ...
    _console.print(
        f'\nGenerate a report:\n  dcs report results {out} --only sim-quality --title "Simulation Quality — {hid}"',
        style="dim",
    )
