...
from dcs_simulation_engine.cli.common import console, done, step
...
def run(
    ...
    # 2) Only one local instance total, regardless of name
    if not deploy:
    ...
    # 3) Branch on whether the run_name already exists
    if status is None:
        # Doesn't exist -> must create (remote or local)
        console.print(f"Creating new run instance: '{run_name}'")
        if deploy:
    ...
    # 4) run_name exists already -> decide whether error or restart
    ...
    if status == STATUS.STOPPED:
        console.print(
            f"Run '{run_name}' already exists but is stopped. Restarting it.",
            style="warning",
        )
        if deploy:
            console.print(
    ...
    # Optional: defensive guard if new statuses ever appear
