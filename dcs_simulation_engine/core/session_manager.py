...
class SessionManager:
    ...
    async def start_persistence(self, *, session_id: str) -> None:
        ...
        self._validation_recorder = ValidationEventRecorder(
            db=db, session_doc=session_doc, primary=self._recorder,
        )
        await self._validation_recorder.__aenter__()
        ensemble = getattr(self.game, "_ensemble", None)
        if ensemble is not None and hasattr(ensemble, "attach_recorder"):
            ensemble.attach_recorder(
